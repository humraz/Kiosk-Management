...
public class resetlogs extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_resetlogs);
        Firebase.setAndroidContext(this);
        read();
    }
    public void read()
    {
        final Firebase ref = new Firebase("https://kioskfarm.firebaseio.com/resets/");
        ...
        ref.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot usersSnapshot) {
                for (DataSnapshot userSnapshot : usersSnapshot.getChildren()) {
                    reset user1 = userSnapshot.getValue(reset.class);
                    loss = user1.getText().toString();
                    stock = user1.getDate().toString();
                    String time= user1.getTime().toString();

                    String st = "Before Reset For " +loss+ "\nTime Reported: "+time;

                    cardslis.add( "Date reported: " + stock );
                    cardsdes.add(st);
